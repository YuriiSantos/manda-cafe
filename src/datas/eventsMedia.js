import eventos5_2 from "../assets/eventos/eventos5.2.jpg";
import eventos9 from "../assets/eventos/eventos9.jpg";
import eventos11 from "../assets/eventos/eventos11.JPG";

import video1 from "../assets/eventos/video1.mp4";
import video2 from "../assets/eventos/video2.mp4";
import video3 from "../assets/eventos/video3.mp4";
import video3_1 from "../assets/eventos/video4.mp4";
import video3_2 from "../assets/eventos/video5.mp4";
import video4 from "../assets/eventos/video6.mp4";
import video4_1 from "../assets/eventos/video7.mp4";
import video5_1 from "../assets/eventos/video8.mp4";
import video6_1 from "../assets/eventos/video9.mp4";
import video9_1 from "../assets/eventos/video10.mp4";

import filmar1 from "../assets/eventos/filmar/1imagem-filmar.jpg";
import filmar2 from "../assets/eventos/filmar/2imagem-filmar.jpg";
import filmar3 from "../assets/eventos/filmar/3imagem-filmar.jpg";
import filmar4 from "../assets/eventos/filmar/4imagem-filmar.jpg";
import filmar5 from "../assets/eventos/filmar/5imagem-filmar.jpg";
import filmar6 from "../assets/eventos/filmar/6video-filmar.mp4";
import filmar7 from "../assets/eventos/filmar/7imagem-filmar.jpg";

import poucos2 from "../assets/eventos/poucos/2imagem-poucos.jpg";
import poucos3 from "../assets/eventos/poucos/3imagem-poucos.jpg";
import poucos4 from "../assets/eventos/poucos/4imagem-poucos.jpg";
import poucos5 from "../assets/eventos/poucos/5imagem-poucos.jpg";
import poucos6 from "../assets/eventos/poucos/6imagem-poucos.jpg";
import poucos7 from "../assets/eventos/poucos/7imagem-poucos.jpg";

export const MEDIA_FESTAS = [
  { type: "video", src: video1 },
  { type: "video", src: video2 },
  { type: "video", src: video3 },
  { type: "video", src: video3_1 },
  { type: "video", src: video3_2 },
  { type: "video", src: video4 },
  { type: "video", src: video4_1 },
  { type: "video", src: video5_1 },
  { type: "video", src: video6_1 },
  { type: "video", src: video9_1 },
  { type: "image", src: eventos11 },
  { type: "image", src: eventos5_2 },
  { type: "image", src: eventos9 },
];

export const MEDIA_POUCOS = [
  { type: "image", src: poucos2 },
  { type: "image", src: poucos3 },
  { type: "image", src: poucos4 },
  { type: "image", src: poucos5 },
  { type: "image", src: poucos6 },
  { type: "image", src: poucos7 },
];

export const MEDIA_FILMAR = [
  { type: "image", src: filmar1 },
  { type: "image", src: filmar2 },
  { type: "image", src: filmar3 },
  { type: "image", src: filmar4 },
  { type: "image", src: filmar5 },
  { type: "video", src: filmar6 },
  { type: "image", src: filmar7 },
];
